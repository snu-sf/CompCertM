...
+ exploit IHlist_forall2; eauto. intros (vl' & B).
  exploit add_equations_builtin_arg_lessdef; eauto. ss.
...
Lemma exec_moves:
  ...
  wt_regset env rs ->
  exists ls',
    star LTL.step tse tge (Block s f sp (expand_moves mv bb) ls m)
...
Inductive match_stackframes: list RTL.stackframe -> list LTL.stackframe -> signature -> Prop :=
  | match_stackframes_nil: forall sg,
      forall sg_init ls_init (SAMERES: sg_init.(sig_res) = sg.(sig_res)),
...
(*       cs cs' sg *)
(*       (MATCH: match_stackframes cs cs' sg) *)
(*   : *)
(*     cs' <> [] *)
(* . *)
