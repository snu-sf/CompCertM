...
Lemma load_stack_transf_ofs
      m sp spofs ty ofs
  :
    <<EQ: load_stack m (Vptr sp spofs true) ty ofs =
          load_stack m (Vptr sp Ptrofs.zero true) ty (Ptrofs.add spofs ofs)>>
.
Proof.
  red. cbn.
...
Lemma store_stack_transf_ofs
      m sp spofs ty ofs v
  :
    <<EQ: store_stack m (Vptr sp spofs true) ty ofs v =
          store_stack m (Vptr sp Ptrofs.zero true) ty (Ptrofs.add spofs ofs) v>>
.
Proof.
  red. cbn.
  rewrite Ptrofs.add_zero_l. ss.
...
Let step := MachC.step return_address_offset.
...
Lemma contains_get_stack:
  ...
  exists v, load_stack m (Vptr sp Ptrofs.zero true) ty (Ptrofs.repr ofs) = Some v /\ spec v.
Proof.
  intros. unfold load_stack.
  replace (Val.offset_ptr (Vptr sp Ptrofs.zero true) (Ptrofs.repr ofs)) with (Vptr sp (Ptrofs.repr ofs) true).
...
Lemma hasvalue_get_stack:
  ...
  load_stack m (Vptr sp Ptrofs.zero true) ty (Ptrofs.repr ofs) = Some v.
Proof.
...
Lemma contains_set_stack:
  ...
  exists m',
      store_stack m (Vptr sp Ptrofs.zero true) ty (Ptrofs.repr ofs) v = Some m'
...
Lemma valid_access_typealign_divides
      bound ty sp sl pos m ls j
      (VALID: forall ofs,
          0 <= ofs ->
...
Proof.
  specialize (VALID 0).
  exploit VALID; eauto; try xomega. { apply Z.divide_0_r. } i; des.
  apply Mem.load_valid_access in H. hnf in H. des. rewrite align_type_chunk in *.
  red. rpapply H1. xomega.
Qed.
...
Lemma frame_get_local:
  ...
  exists v,
     load_stack m (Vptr sp Ptrofs.zero true) ty (Ptrofs.repr (offset_local fe ofs)) = Some v
...
Lemma frame_get_parent:
  ...
  m |= frame_contents j sp ls ls0 parent retaddr ** P ->
  load_stack m (Vptr sp Ptrofs.zero true) Tptr (Ptrofs.repr fe.(fe_ofs_link)) = Some parent.
Proof.
...
Lemma frame_get_retaddr:
  forall j sp ls ls0 parent retaddr m P,
  m |= frame_contents j sp ls ls0 parent retaddr ** P ->
  load_stack m (Vptr sp Ptrofs.zero true) Tptr (Ptrofs.repr fe.(fe_ofs_retaddr)) = Some retaddr.
Proof.
  unfold frame_contents, frame_contents_1; intros.
...
Lemma frame_set_local:
  ...
  Val.inject j v v' ->
  exists m',
     store_stack m (Vptr sp Ptrofs.zero true) ty (Ptrofs.repr (offset_local fe ofs)) v' = Some m'
...
Lemma frame_set_outgoing:
  ...
  Val.inject j v v' ->
  exists m',
     store_stack m (Vptr sp Ptrofs.zero true) ty (Ptrofs.repr (fe_ofs_arg + (offset_arg ofs))) v' = Some m'
...
Proof.
  ...
  unfold slot_valid in H1; InvBooleans. simpl in H0.
  (* refine (modusponens _ _ (set_location _ _ _ fe_ofs_arg _ _ _ _ _ _ _ _ _ _ _ _ _) _); eauto. intros (m' & A & B). *)
  exploit set_location; eauto. intros (m' & A & B).
  exists m'; split; auto.
  (* rewrite <- store_stack_transf_ofs. *)
  assert (forall i k p, Mem.perm m sp i k p -> Mem.perm m' sp i k p).
  {  intros. unfold store_stack in A; simpl in A. eapply Mem.perm_store_1; eauto. }
  eapply frame_mconj. eauto.
...
Lemma undef_regs_outside:
  forall ml ls l,
  Loc.notin l (map R ml) -> LTL.undef_regs ml ls l = ls l.
Proof.
  induction ml; simpl; intros. auto.
  rewrite Locmap.gso. apply IHml. tauto. apply Loc.diff_sym. tauto.
Qed.

Local Opaque all_mregs. (* Note: without it, "Qed." does not end! Is it a bug? *)

Lemma agree_callee_save_extcall:
  forall ls1 ls2,
  agree_callee_save ls1 ls2 ->
  agree_callee_save (LTL.undef_regs destroyed_at_call ls1) ls2.
Proof.
  repeat intro.
  specialize (H l).
  destruct l; auto.
  erewrite <- H; eauto.
  eapply undef_regs_outside; eauto.
  apply Loc.notin_iff. intros.
  rewrite in_map_iff in H1.
  destruct H1 as [mr [R IN]].
  symmetry in R; subst.
  repeat intro. subst.
  unfold destroyed_at_call in *.
  apply filter_In in IN. destruct IN as [IN NEG].
  rewrite H0 in *. inv NEG.
Qed.

Local Transparent all_mregs.
...
Lemma save_callee_save_rec_correct:
  ...
  exists rs', exists m',
     star step tge
        (State cs fb (Vptr sp Ptrofs.zero true) (save_callee_save_rec l pos k) rs m)
     E0 (State cs fb (Vptr sp Ptrofs.zero true) k rs' m')
...
Lemma save_callee_save_correct:
  ...
  exists rs', exists m',
     star step tge
        (State cs fb (Vptr sp Ptrofs.zero true) (save_callee_save fe k) rs1 m)
     E0 (State cs fb (Vptr sp Ptrofs.zero true) k rs' m')
...
Lemma function_prologue_correct:
  ...
  /\ store_stack m2' (Vptr sp' Ptrofs.zero true) Tptr tf.(fn_link_ofs) parent = Some m3'
  /\ store_stack m3' (Vptr sp' Ptrofs.zero true) Tptr tf.(fn_retaddr_ofs) ra = Some m4'
  /\ star step tge
         (State cs fb (Vptr sp' Ptrofs.zero true) (save_callee_save fe k) rs1 m4')
      E0 (State cs fb (Vptr sp' Ptrofs.zero true) k rs' m5')
  /\ agree_regs j' ls1 rs'
  /\ agree_locs ls1 ls0
  /\ m5' |= frame_contents 0 j' sp' ls1 ls0 parent ra ** minjection j' m2 ** globalenv_inject ge j' ** P
...
Proof.
  ...
  (* Stack layout info *)
  assert(FE: fe = make_env b). { auto. }
Local Opaque b fe.
  generalize (frame_env_range 0 b) (frame_env_aligned b 0). replace (make_env_ofs 0 b) with fe by auto. simpl.
  intros LAYOUT1 LAYOUT2.
  special LAYOUT1. { omega. } special LAYOUT2. { apply Z.divide_0_r. } clear H H0.
  (* Allocation step *)
  ...
  rewrite sep_swap in SEP.
  (* Now we frame this *)
  assert (SEPFINAL: m5' |= frame_contents 0 j' sp' ls1 ls0 parent ra ** minjection j' m2 ** globalenv_inject ge j' ** P).
...
Lemma restore_callee_save_rec_correct:
  ...
  exists rs',
    star step tge
      (State cs fb (Vptr sp Ptrofs.zero true) (restore_callee_save_rec l ofs k) rs m)
   E0 (State cs fb (Vptr sp Ptrofs.zero true) k rs' m)
...
Lemma restore_callee_save_correct:
  ...
  m |= frame_contents 0 j sp ls ls0 pa ra ** P ->
  agree_unused j ls0 rs ->
  exists rs',
    star step tge
       (State cs fb (Vptr sp Ptrofs.zero true) (restore_callee_save fe k) rs m)
    E0 (State cs fb (Vptr sp Ptrofs.zero true) k rs' m)
  /\ (forall r,
...
Lemma function_epilogue_correct:
  ...
  m' |= frame_contents 0 j sp' ls ls0 pa ra ** minjection j m ** P ->
  agree_regs j ls rs ->
  agree_locs ls ls0 ->
  j sp = Some(sp', fe.(fe_stack_data)) ->
  Mem.free m sp 0 f.(Linear.fn_stacksize) = Some m1 ->
  exists rs1, exists m1',
     load_stack m' (Vptr sp' Ptrofs.zero true) Tptr tf.(fn_link_ofs) = Some pa
  /\ load_stack m' (Vptr sp' Ptrofs.zero true) Tptr tf.(fn_retaddr_ofs) = Some ra
  /\ Mem.free m' sp' 0 tf.(fn_stacksize) = Some m1'
  /\ star step tge
       (State cs fb (Vptr sp' Ptrofs.zero true) (restore_callee_save fe k) rs m')
    E0 (State cs fb (Vptr sp' Ptrofs.zero true) k rs1 m')
...
+ simpl in SEP. unfold parent_sp.
  assert (slot_valid (dummy_function sg) Outgoing pos ty = true).
  { destruct H0. unfold slot_valid, proj_sumbool.
    rewrite zle_true by omega. rewrite pred_dec_true by auto. reflexivity. }
  assert (slot_within_bounds (function_bounds (dummy_function dummy_sig)) Outgoing pos ty).
  { apply ARGS; ss. }
  des_ifs.
  exploit frame_get_outgoing; eauto. intros (v & A & B).
  ss. u in Heq. des_ifs.
  exists v; split.
  constructor. rewrite Ptrofs.repr_unsigned in A. exact A. red in AGCS. rewrite AGCS; auto.
+ simpl in SEP. unfold parent_sp.
...
Hypothesis AGR: agree_regs j ls rs.
Hypothesis SEP: m' |= frame_contents f 0 j sp' ls ls0 parent retaddr ** minjection j m ** globalenv_inject ge j.

Lemma transl_builtin_arg_correct:
  forall a v,
  eval_builtin_arg ge ls (Vptr sp Ptrofs.zero true) m a v ->
  ...
  exists v',
     eval_builtin_arg ge rs (Vptr sp' Ptrofs.zero true) m' (transl_builtin_arg fe a) v'
  /\ Val.inject j v v'.
Proof.
...
- set (ofs' := Ptrofs.add ofs (Ptrofs.repr (fe_stack_data fe))).
  ...
  instantiate (1 := Val.offset_ptr (Vptr sp' Ptrofs.zero true) ofs').
...
Lemma transl_builtin_args_correct:
  forall al vl,
  eval_builtin_args ge ls (Vptr sp Ptrofs.zero true) m al vl ->
  ...
  exists vl',
     eval_builtin_args ge rs (Vptr sp' Ptrofs.zero true) m' (List.map (transl_builtin_arg fe) al) vl'
...
Inductive match_states: Linear.state -> Mach.state -> Prop :=
  | match_states_intro:
      forall cs f sp c ls m cs' fb sp' rs m' j tf
        ...
        (TAIL: is_tail c (Linear.fn_code f))
        (SEP: m' |= frame_contents f 0 j sp' ls (parent_locset cs) (parent_sp cs') (parent_ra cs')
                 ** stack_contents j cs cs'
                 ** minjection j m
                 ** globalenv_inject ge j),
      match_states (Linear.State cs f (Vptr sp Ptrofs.zero true) c ls m)
                   (Mach.State cs' fb (Vptr sp' Ptrofs.zero true) (transl_code (make_env (function_bounds f)) c) rs m')
  | match_states_call:
...
+ (* Lgetstack, incoming *)
  ...
  inversion STACKS; clear STACKS.
  {
    ...
    rewrite make_env_ofs_zero.
    eapply frame_get_parent. eexact SEP.
    rewrite Ptrofs.repr_unsigned in A. ss. eauto.
...
- (* Lsetstack *)
  ...
  assert (A: exists m'',
              store_stack m' (Vptr sp' Ptrofs.zero true) ty (Ptrofs.repr ofs') (rs0 src) = Some m''
           /\ m'' |= frame_contents f 0 j sp' (Locmap.set (S sl ofs ty) (rs (R src))
...
- (* Lop *)
  assert (exists v',
          eval_operation ge (Vptr sp' Ptrofs.zero true) (transl_op (make_env (function_bounds f)) op) rs0##args m' = Some v'
       /\ Val.inject j v v').
...
- (* Lload *)
  assert (exists a',
          eval_addressing ge (Vptr sp' Ptrofs.zero true) (transl_addr (make_env (function_bounds f)) addr) rs0##args = Some a'
       /\ Val.inject j a a').
...
- (* return *)
  inv STACKS. { ss. } simpl in AGLOCS. simpl in SEP. rewrite sep_assoc in SEP.
  econstructor; split.
...
(*   red; simpl; auto. *)
(*   red; simpl; auto. *)
(*   { eapply Mem.flat_inj_ptr; eauto. exploit Genv.find_symbol_not_fresh; eauto. } *)
