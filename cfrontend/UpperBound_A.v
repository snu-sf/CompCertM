...
Section PRESERVATION.
  ...
  Notation " 'geof' cp" := (Build_genv (CSkEnv.revive (SkEnv.project skenv_link (defs cp)) cp) cp.(prog_comp_env))
...
Let geof := fun skenv_link (cp: Csyntax.program) =>
              (Build_genv (CSkEnv.revive (SkEnv.project skenv_link (defs cp)) cp) cp.(prog_comp_env)).
