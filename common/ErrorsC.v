...
Lemma mmap_partial A B C (f: A -> res B) (g: B -> res C) la lc
  ...
  :
    exists lb,
      (<<MMAP0: mmap f la = OK lb>>) /\
      (<<MMAP1: mmap g lb = OK lc>>).
Proof.
  ginduction la; ss; i; clarify.
  - esplits; eauto.
  - unfold bind in *. des_ifs_safe.
    exploit IHla; eauto. i. des.
    rewrite MMAP0. esplits; eauto.
    ss. unfold bind. rewrite MMAP1. des_ifs.
Qed.
