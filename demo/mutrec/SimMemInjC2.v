...
(* Variable P : block -> ptrofs -> Prop. *)
  Variable P : memory_chunk -> Z -> val -> Prop.
    
  Record t' := mk
    { minj:> SimMemInj.t';
      inv_range: block -> Prop; }.
  ...
  | wf_intro
      (WF: SimMemInj.wf' sm0)
      (SAT: forall blk chunk ofs v (IN: sm0.(inv_range) blk)
                   (LOAD: Mem.load chunk sm0.(SimMemInj.tgt) blk ofs = Some v),
          P chunk ofs v)
      (INVRANGE: forall blk ofs (RANGE: sm0.(inv_range) blk),
          tgt_private sm0 blk ofs)
  . 

  (* "Global" is needed as it is inside section *)
  Global Program Instance SimMemInjPred : SimMem.class :=
    {
      t := t';
      src := src;
      tgt := tgt;
