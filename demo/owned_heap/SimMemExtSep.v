...
Lemma store_left_pub
      sm0 chunk v_tgt ofs blk m_src1
      (MWF: wf' sm0)
      (PMPUB: brange blk (ofs) (ofs + size_chunk chunk) <2= (ons_mem sm0.(ptt) etc))
      (UNDEFSRC: Mem.loadbytes sm0.(src) blk ofs (size_chunk chunk) =
                 Some (list_repeat (Z.to_nat (size_chunk chunk)) Undef))
      (STRSRC: Mem.store chunk sm0.(src) blk (ofs) v_tgt = Some m_src1)
      (LDTGT: Mem.load chunk sm0.(tgt) blk ofs = Some v_tgt)
:
    exists sm1,
      (<<MSRC: sm1.(src) = m_src1>>)
      /\ (<<MTGT: sm1.(tgt) = sm0.(tgt)>>)
...
Proof.
  ...
  - econs; ss; eauto.
    ...
    etrans; et.
Qed.
Lemma store_right_pub
      sm0 chunk v_tgt ofs blk m_tgt0
      (MWF: wf' sm0)
      (PMPUB: brange blk (ofs) (ofs + size_chunk chunk) <2= (ons_mem sm0.(ptt) etc))
      (UNDEFSRC: Mem.loadbytes sm0.(src) blk ofs (size_chunk chunk) =
                 Some (list_repeat (Z.to_nat (size_chunk chunk)) Undef))
