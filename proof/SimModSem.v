...
(* Arguments sm_match_tmp: clear implicits. *)
  ...
  Proof.
    ...
    { destruct n; ss. }
    rename msp into mspA.
    rr in smos. destruct smos as [smos0 smos1].
    dependent destruction smos1.
    rename msp into mspB.
    Set Printing Implicit.
    (* MSP0: mspB
       hd: mspB
       MWF: mspB
    ...
    -
      ...
      { Check ((smos0, smos1): SimMemOhs_t msps).
        instantiate (1:= (smos0, smos1)).
    i. assert(msps = []).
    { destruct msps; ss. }
    ii. clarify. ss.
    generalize msps.
    dependent destruction msps. hrewrite H.
    destruct msps.
    generalize msps.
    Set Printing All.
  Qed.

  Goal forall n msp (MSP: nth_error msps n = Some msp),
      (<<SMPROJ: forall smos (MWF: SimMemOhs_wf smos),
          exists smo, sm_match_tmp msp smo smos /\ SimMemOh.wf smo>>).
  Proof.
    clear SimMemOhs_le.
    (* Set Printing All. *)
    intros ? ? ?.
    ...
    generalize smos.
    Set Printing Implicit.
    dependent destruction msps.
    Set Printing All.
    ii.
    { Set Printing All.
      generalize (@ModSemPair.SMO SM SS msp).
      generalize (@ModSemPair.src SM SS msp). destruct msps.
    ii.
  Qed.

  Goal forall m n msp (LT: (n < m)%nat) (MSP: nth_error msps n = Some msp),
      (<<SMPROJ: forall sm (smos_aux: SimMemOhs_t_aux m)
                        (MWF: SimMemOhs_wf_aux sm smos_aux),
          exists (smo: SimMemOh.t),
            sm = smo.(SimMemOh.sm) /\
            (projT1 ((SimMemOhs_ohs_src_aux smos_aux)
                       (msp.(ModSemPair.src).(midx)))) =
             msp.(ModSemPair.src).(owned_heap) /\
            (projT2 ((SimMemOhs_ohs_src_aux smos_aux)
                       (msp.(ModSemPair.src).(midx)))
                   ~= smo.(SimMemOh.oh_src)) /\
            SimMemOh.wf smo>>).
  Proof.
    intro. induction m; ii; ss.
    { inv MWF. simpl_depind. clarify. dependent destruction smos_aux.
      omega. }
    (* - destruct n; ss. *)
    (*   { des_ifs. exists sm. esplits; eauto. } *)
    - dependent destruction smos_aux.
      rename msp into msp_n. rename msp0 into msp_m.
      destruct n; ss; cycle 1.
      { exploit (IHm n); eauto. { omega. }
      }
      { revert MSP0 MWF.
        revert hd.
        revert msp_m.
        generalize (nth_error msps m = Some msp_m).
        destruct msps. des_ifs. exists sm. esplits; eauto.
        -
      }
  Qed.
