...
Section SIMMODSEM.
  ...
  (*   mcompat_src: <<MCOMPATSRC: ms_src.(get_mem) st_src0 = (SimMem.src sm0)>>; *)
  (*   mcompat_tgt: <<MCOMPATTGT: ms_tgt.(get_mem) st_tgt0 = (SimMem.tgt sm0)>>; *)
  (* } *)
  (* . *)
...
(* Note: We require le for mle_preserves_sim_ge, but we cannot require SimMem.wf, beacuse of DCEproof *)
  ...
  (*     (RSREL: (SimMem.sim_regset sm0) rs_arg_src rs_arg_tgt) *)
  (*     (VALID: SimMem.wf sm0) *)
  (*     (AFTER: forall *)
  (*         sm1 rs_ret_src rs_ret_tgt *)
  (*         (MLE: SimMem.le (SimMem.lift sm0) sm1) *)
  (*         (VALID: SimMem.wf sm1) *)
  (*         (RETVREL: (SimMem.sim_regset sm1) rs_ret_src rs_ret_tgt) *)
  (*         st_tgt1 *)
  (*         (AFTERTGT: ms_tgt.(after_external) st_tgt0 rs_arg_tgt rs_ret_tgt (SimMem.tgt sm1) *)
  (*                                                                                st_tgt1) *)
  (*       , *)
  (*         exists i1 st_src1, *)
  (*         (<<AFTERSRC: ms_src.(after_external) st_src0 rs_arg_src rs_ret_src (SimMem.src sm1) *)
  (*                                                                                  st_src1>>) *)
  (*         /\ *)
