...
Section DENOTE.
  ...
  Definition denote_function: (InternalCallE ~> itree E) :=
    fun T ei =>
  ...
  (* Definition interp_MemE {E A} (t: itree (MemE +' E) A) (m0: mem): itree E A := *)
  ...
  (* Definition interp_OwnedHeapE {E A} (t: itree (OwnedHeapE +' E) A) (oh0: owned_heap): itree E A := *)
  (*   let t': stateT owned_heap (itree E) A := interp_state (case_ handle_OwnedHeapE pure_state) t in *)
  (*   '(_, a) <- t' oh0 ;; Ret a *)
  (* . *)

  Definition denote_program2: (InternalCallE ~> itree eff2) :=
    ...
    fun _ ic =>
      ...
      let sem2: itree eff2 _ := interp_LocalE sem3 in
      sem2
  .

  Definition denote_program3: (InternalCallE ~> itree eff3) :=
    fun _ ic =>
      let sem4: itree eff4 _ := (mrec denote_function) _ ic in
      let sem3: itree eff3 _ := interp_GlobalE sem4 in
      sem3
