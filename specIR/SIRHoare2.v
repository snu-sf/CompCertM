...
Section RET.
  ...
  Inductive _returns (returns: itree E R -> Prop): itree E R -> Prop :=
  | returns_ret
      r
      (RET: RP r)
    :
      _returns returns (Ret r)
  | returns_tau
      itr
      (RET: returns itr)
    :
      _returns returns (Tau itr)
  | returns_vis
      X (e: E X) ktr
      (RET: forall (x: X), returns (ktr x))
    :
      _returns returns (Vis e ktr)
  .
  Definition returns: itree E R -> Prop := paco1 _returns bot1.
  Lemma returns_mon: monotone1 _returns.
  Proof.
    ii. inv IN; try (by econs; et; rr; et).
  Qed.
  Hint Unfold returns.
  Hint Resolve returns_mon: paco.

  Theorem returns_bind
          X itr ktr
          (RET: forall (x: X), returns (ktr x))
    :
      <<RET: returns (x <- itr ;; ktr x)>>
  .
  Proof.
    revert_until RP. ginit.
    { intros. eapply cpn1_wcompat; eauto with paco. }
    gcofix CIH.
    ii. ides itr; irw.
    - gfinal. right.
      eapply paco1_mon.
    ...
    - gstep. econs; et. ii. eauto with paco.
  Qed.
End RET.
Hint Unfold returns.
Hint Resolve returns_mon: paco.
...
| match_icall_fn
    ...
    (PRECOND: precond oh0 m0 vs0)
    (MATCH: forall
        ohmv
        (POSTCOND: postcond oh0 m0 vs0 ohmv)
      ,
        match_itr (k_src ohmv) (k_tgt ohmv))
  :
    _match_itr match_itr
               (trigger (ICall _fn_ru oh0 m0 vs0) >>= k_src)
               (trigger (ICall _fn_ru oh0 m0 vs0) >>= k_tgt)
| match_ecall
    sg oh m vs fptr
    k_src
    k_tgt
...
| match_fn_intro
    (SIM: (eq ==> eq ==> eq ==> match_itr) fn_ru fn_tgt)
    (HOARE: forall
        oh0 m0 vs0
        (PRECOND: precond oh0 m0 vs0)
      ,
        <<PSOTCOND: returns (postcond oh0 m0 vs0) (fn_tgt oh0 m0 vs0)>>)
.
...
Proof.
  ...
  - rewrite ! bind_bind. gstep.
    econs; ss.
    ii. exploit MATCH; et. i; des. pclearbot. eauto with paco.
  - rewrite ! bind_vis. gstep. econs; eauto. ii. clarify.
    repeat spc MATCH. hexploit1 MATCH; ss. pclearbot. eauto with paco.
