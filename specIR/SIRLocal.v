...
Inductive TauC2 (r: itr_src -> itr_tgt -> Prop) : itr_src -> itr_tgt -> Prop :=
| TauC2_base
    i_src i_tgt
    (REL: r i_src i_tgt)
  :
...
| TauC2_left
    i_src i_tgt
    (REL: TauC2 r i_src i_tgt)
  :
    TauC2 r (Tau i_src) i_tgt
| TauC2_right
    i_src i_tgt
    (REL: TauC2 r i_src i_tgt)
  :
    TauC2 r i_src (Tau i_tgt)
| TauC2_right_rev
    i_src i_tgt
    (REL: TauC2 r i_src (Tau i_tgt))
  :
    TauC2 r i_src i_tgt
.
Hint Constructors TauC2.
...
Lemma TauC2_wcompat
  :
    wcompatible2 _match_itr TauC2.
Proof.
  constructor.
  { intros x0 x1 r r' IN LE. induction IN; subst; eauto. }
  intros.
  hexploit (case_analysis x1). i; des; cycle 1.
  - clarify. remember (ITree.spin) as i_src. revert Heqi_src. induction PR; i; subst.
    { eauto with paco. }
    { econs; et. }
    { rewrite unfold_spin in *. clarify. rewrite <- unfold_spin in *. eauto with paco. }
    { admit "". }
  - move m at top. revert_until r. induction m; i; ss.
    + inv CONVERGE; ss.
      { remember (Ret r0) as i_src. revert Heqi_src. induction PR; ii; ss; clarify.
        { eauto with paco. }
        { econs; et. }
        { econs; et. }
        { econs; et. }
        econs; et. inv PR; ss.
    rewrite unfold_spin in *. clarify. rewrite <- unfold_spin in *. eauto with paco. }
    rewrite unfold_spin. econs; et. }
  induction PR; subst.
  { eauto with paco. }
  { econs; eauto. }
  { econs; eauto. }
  hexploit (case_analysis i_src). i; des.
  - induction m; ii; ss.
    + inv CONVERGE.
      * inv IHPR. inv SIM; econs; et.
      * inv IHPR; csc.
        { econs; et. }
        { des. econs; et. eauto with paco. }
        inv SIM; econs; et.
      
    inv IHPR; ss.
    { econs; et.
      clear - IHPR.
      remember (tau;; i_tgt) as i_tgt0. revert Heqi_tgt0. induction IHPR; i; ss; clarify.
